    ...
    public static class ValidatorRules
    {
                ...
                field.Equals("ESPECIE_DA_PERICIA", StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (hasEmail || hasCpf || hasNoise || looksPerson)
                {
                    if (Environment.GetEnvironmentVariable("OPERPDF_VAL_DEBUG") == "1")
                        Console.WriteLine($"[VALDBG-core] field={field} reject email={hasEmail} cpf={hasCpf} noise={hasNoise} person={looksPerson} value=\"{TextUtils.NormalizeWhitespace(value)}\"");
                    return false;
                }
                ...
                {
                    if (Environment.GetEnvironmentVariable("OPERPDF_VAL_DEBUG") == "1")
                        Console.WriteLine($"[VALDBG-core] field={field} accept=external_validator value=\"{TextUtils.NormalizeWhitespace(value)}\"");
                    return true;
                }
                ...
                {
                    if (Environment.GetEnvironmentVariable("OPERPDF_VAL_DEBUG") == "1")
                        Console.WriteLine($"[VALDBG-core] field={field} accept=fallback_regex value=\"{TextUtils.NormalizeWhitespace(value)}\"");
                    return true;
                }
                ...
                if (Environment.GetEnvironmentVariable("OPERPDF_VAL_DEBUG") == "1")
                    Console.WriteLine($"[VALDBG-core] field={field} accept=loose:{okLoose.ToString().ToLowerInvariant()} value=\"{fallback}\"");
                return okLoose;
            }
            ...
            out string reason)
        {
            ...
            if (isRequerimento)
            {
                ...
                var strictKnownKeys = new HashSet<string>(GetSupportedFieldValidationKeys(), StringComparer.OrdinalIgnoreCase);
                strictKnownKeys.ExceptWith(new[] { "PROCESSO_ADMINISTRATIVO", "DATA_REQUISICAO", "PROCESSO_JUDICIAL" });
                foreach (var key in strictKnownKeys.OrderBy(x => x, StringComparer.OrdinalIgnoreCase))
                {
                    if (!TryGetNonEmpty(key, out var v))
                        continue;
                    if (!isValueValidForField(key, v, catalog, out var why))
                    {
                        reason = $"{key}:{why}";
                        return true;
                    }
            ...
            if (isCertidao)
            {
                ...
                var strictKnownKeys = new HashSet<string>(GetSupportedFieldValidationKeys(), StringComparer.OrdinalIgnoreCase);
                strictKnownKeys.ExceptWith(new[] { "PROCESSO_ADMINISTRATIVO", "VALOR_ARBITRADO_CM", "DATA_AUTORIZACAO_CM", "PERITO" });
                foreach (var key in strictKnownKeys.OrderBy(x => x, StringComparer.OrdinalIgnoreCase))
                {
                    if (!TryGetNonEmpty(key, out var v))
                        continue;
            ...
            var strictAllKnown = new HashSet<string>(GetSupportedFieldValidationKeys(), StringComparer.OrdinalIgnoreCase);
            strictAllKnown.ExceptWith(core);
            foreach (var key in strictAllKnown.OrderBy(x => x, StringComparer.OrdinalIgnoreCase))
            {
                if (!TryGetNonEmpty(key, out var extra))
                    continue;
                if (!isValueValidForField(key, extra, catalog, out var extraWhy))
                {
                    if ((key.Equals("ESPECIALIDADE", StringComparison.OrdinalIgnoreCase) ||
                         key.Equals("ESPECIE_DA_PERICIA", StringComparison.OrdinalIgnoreCase)) &&
                        string.Equals(extraWhy, "format_invalid", StringComparison.OrdinalIgnoreCase))
                    {
                        var soft = TextUtils.NormalizeWhitespace(extra);
                        if (soft.Length >= 4 && Regex.IsMatch(soft, @"[\p{L}]"))
                            continue;
                    }
        ...
        {
            return ValidationCatalog.GetSupportedFieldValidationKeys();
        }

        public static IReadOnlyList<string> GetSupportedDocumentValidationProfiles()
        {
            return ValidationCatalog.GetSupportedDocumentValidationProfiles();
        }
            ...
            out string reason)
        {
            ...
            {
                reason = "values_null";
                flow.Add("fail:values_null");
                return flow;
            }

            flow.Add("output_doc_type=" + outputDocType);

            var patternsPath = ResolvePatternPathForOutputDocType(outputDocType);
            if (string.IsNullOrWhiteSpace(patternsPath) || !File.Exists(patternsPath))
            {
                reason = "pattern_not_found";
                flow.Add("fail:pattern_not_found");
                return flow;
            }

            flow.Add("pattern=" + Path.GetFileName(patternsPath));
            ...
            flow.Add("input_non_empty=" + (local.Count == 0
                ? "(nenhum)"
                : string.Join(", ", local.OrderBy(k => k.Key, StringComparer.OrdinalIgnoreCase)
                    .Select(kv => $"{kv.Key}={ShortTrace(kv.Value)}"))));
            ...
            {
                local["DATA_AUTORIZACAO_CM"] = certDate;
                flow.Add("bridge:DATA_AUTORIZACAO_CM<=DATA_ARBITRADO_FINAL");
            }
            ...
            flow.Add("optional_fields=" + string.Join(",", optional.OrderBy(x => x, StringComparer.OrdinalIgnoreCase)));
            flow.Add("doc_key=" + DocumentValidationRules.ResolveDocKeyFromPatternPath(patternsPath));
            ...
            var reject = ShouldRejectByValidator(
                ...
                (string field, string value, PeritoCatalog? peritoCatalog, out string why) =>
                {
                    var valid = IsValueValidForField(field, value, peritoCatalog, null, null, out why);
                    flow.Add(valid
                        ? $"{field}:ok value={ShortTrace(value)}"
                        : $"{field}:{why} value={ShortTrace(value)}");
                    return valid;
                },
                out reason);

            if (!string.IsNullOrWhiteSpace(reason))
                flow.Add("decision=" + reason);
            else
                flow.Add("decision=ok");
        ...
        private static string ShortTrace(string value, int max = 42)
        {
            if (string.IsNullOrWhiteSpace(value))
                return "\"\"";
            var t = TextUtils.NormalizeWhitespace(value);
            if (t.Length <= max)
                return "\"" + t + "\"";
